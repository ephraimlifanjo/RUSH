...
function resolveEngine() {
  ...
  const candidates = [
    process.env.RUSH_PYTHON_EXE,
    process.platform === 'win32' ? path.join(__dirname, '..', '.venv', 'Scripts', 'python.exe') : path.join(__dirname, '..', '.venv', 'bin', 'python'),
...
function runEngine(tool, payload = {}, timeoutMs = 20 * 60 * 1000) {
  return new Promise((resolve, reject) => {
    let engine;
    try { engine = resolveEngine(); } catch (error) { reject(error); return; }
    const child = spawn(engine.cmd, [...engine.args, tool], {
      windowsHide: true,
      shell: false,
      stdio: ['pipe','pipe','pipe'],
      env: { ...process.env, RUSH_USER_DATA: app.getPath('userData') }
    });
    let stdout = '';
    let stderr = '';
    let settled = false;
    ...
    child.stdout.on('data', chunk => {
      ...
      if (stdout.length > 40_000_000) { try { child.kill(); } catch {} finish(reject, new Error('Engine response exceeded safety limit.')); }
    });
    child.stderr.on('data', chunk => { stderr += chunk.toString(); if (stderr.length > 5_000_000) stderr = stderr.slice(-5_000_000); });
    child.on('error', error => finish(reject, error));
    child.on('close', code => {
      if (settled) return;
      if (code !== 0) return finish(reject, new Error(stderr.trim() || `Engine exited with code ${code}`));
      try {
        const result = JSON.parse(stdout.trim() || '{}');
        if (result.ok === false) finish(reject, new Error(result.error || `${tool} failed`)); else finish(resolve, result);
      } catch { finish(reject, new Error(`Invalid engine response: ${stdout.slice(0, 700)}`)); }
    });
...
function resolveNativeCore() {
  ...
  const candidates = app.isPackaged
    ...
    : [path.join(__dirname, '..', 'resources', 'native', name), path.join(__dirname, '..', 'native', 'build', name)];
  return candidates.find(p => fs.existsSync(p)) || null;
}
function nativeScan(root) {
  const exe = resolveNativeCore();
  if (!exe) return Promise.resolve(null);
  return new Promise(resolve => {
    const child = spawn(exe, ['scan', root], { windowsHide:true, shell:false });
    let stdout = '';
    child.stdout.on('data', d => stdout += d.toString());
    child.on('error', () => resolve(null));
    child.on('close', code => { if (code !== 0) return resolve(null); try { resolve(JSON.parse(stdout)); } catch { resolve(null); } });
  });
}

function fileFromArgs(argv) {
  return argv.find(value => typeof value === 'string' && SUPPORTED.has(path.extname(value).toLowerCase()) && fs.existsSync(value)) || null;
}
...
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1540, height: 960, minWidth: 1080, minHeight: 720,
    title: 'RUSH Office Suite', backgroundColor: '#f5f5f5', show: false,
  ...
  mainWindow.webContents.on('did-finish-load', () => { if (pendingOpen) { const file = pendingOpen; pendingOpen = null; sendOpenFile(file); } });
...
if (!gotLock) app.quit();
else app.on('second-instance', (_event, argv) => sendOpenFile(fileFromArgs(argv)));
app.on('open-file', (event, filePath) => { event.preventDefault(); sendOpenFile(filePath); });
app.whenReady().then(() => {
  Menu.setApplicationMenu(null);
  pendingOpen = fileFromArgs(process.argv) || pendingOpen;
  createWindow();
  app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createWindow(); });
});
app.on('window-all-closed', () => { if (process.platform !== 'darwin') app.quit(); });

ipcMain.handle('dialog:open', async (event, kind = 'document') => {
  safeSender(event);
  const filters = kind === 'pdf'
    ? [{ name:'PDF', extensions:['pdf'] }]
    : [{ name:'RUSH Documents', extensions:['pdf','doc','docx','odt','rtf','txt','html','htm'] }];
  const result = await dialog.showOpenDialog(mainWindow, { properties:['openFile'], filters });
  return result.canceled ? null : result.filePaths[0];
});
ipcMain.handle('dialog:open-many', async (event, extensions = ['pdf']) => {
  safeSender(event);
  const result = await dialog.showOpenDialog(mainWindow, { properties:['openFile','multiSelections'], filters:[{ name:'Files', extensions }] });
...
ipcMain.handle('library:discover', async (event, roots) => {
  safeSender(event);
  const result = [];
  for (const root of roots || []) {
    if (!root || !fs.existsSync(root)) continue;
    const native = await nativeScan(root);
    if (Array.isArray(native)) result.push(...native.map(x => x.path || x).filter(Boolean));
    else {
      const stack = [root];
      while (stack.length && result.length < 50_000) {
        const dir = stack.pop();
        let entries = [];
        try { entries = fs.readdirSync(dir, { withFileTypes:true }); } catch { continue; }
        for (const entry of entries) {
          const full = path.join(dir, entry.name);
          if (entry.isDirectory()) { if (!['node_modules','.git','Windows','$Recycle.Bin'].includes(entry.name)) stack.push(full); }
